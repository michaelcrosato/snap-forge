...
BEGIN
    ...
    -- Lock the idempotency row to prevent race conditions from concurrent executions
    ...
    IF FOUND THEN
        IF v_idemp_status IN ('SUCCESS', 'PENDING_APPROVAL') THEN
            ...
            RETURN v_result;
        ELSIF v_idemp_status = 'PENDING' THEN
            RAISE EXCEPTION 'Request is already in progress' USING ERRCODE = '42000';
        END IF;
    ELSE
        -- Insert pending key to lock key, wrapping in subtransaction to handle concurrent inserts
        BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        -- Insert/Update failure in idempotency keys table even if early insert rolled back
...
GRANT EXECUTE ON FUNCTION inventory.adjust_quantity(UUID, INT, TEXT, TEXT) TO authenticated;

-- Create gateway executor role
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA inventory, gateway, audit TO gateway_executor;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.tenants, public.tenant_users, public.outbox_events TO gateway_executor;

GRANT SELECT ON TABLE public.tenant_users TO gateway_executor;
GRANT SELECT ON TABLE public.tenants TO gateway_executor;

ALTER FUNCTION inventory.adjust_quantity(UUID, INT, TEXT, TEXT) OWNER TO gateway_executor;
...
CREATE POLICY gateway_all_audit_logs ON audit.audit_logs
    FOR ALL TO gateway_executor
    USING (tenant_id = nullif(current_setting('gateway.current_tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = nullif(current_setting('gateway.current_tenant_id', true), '')::uuid);

CREATE POLICY gateway_all_tenants ON public.tenants
    FOR ALL TO gateway_executor
    USING (id = nullif(current_setting('gateway.current_tenant_id', true), '')::uuid)
    WITH CHECK (id = nullif(current_setting('gateway.current_tenant_id', true), '')::uuid);

CREATE POLICY gateway_all_tenant_users ON public.tenant_users
    FOR ALL TO gateway_executor
    USING (tenant_id = nullif(current_setting('gateway.current_tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = nullif(current_setting('gateway.current_tenant_id', true), '')::uuid);

-- Lockdown Policies (Redundant Permissive Policies) for authenticated, anon, and PUBLIC
